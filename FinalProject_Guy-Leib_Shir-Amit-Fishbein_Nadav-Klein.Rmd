...
brca <-read.csv("C:/Users/leibguy/Desktop/data/brca.csv")
```
```{r cleaning}
...
# Set the number of iterations and k values
num_iterations <- 100
k_values <- c(3, 5, 7, 9, 11,15,18,21,25)

# Initialize a vector to store accuracies for each k value
accuracies <- numeric(length(k_values))

# Initialize variables for storing best k value and its corresponding confusion matrix
best_k <- 0
best_accuracy <- 0
...
for (i in 1:num_iterations) {
  ...
  # Create index to split the shuffled data into train and test sets
  ...
  # Train a model for each k value
  for (j in 1:length(k_values)) {
    # Train a KNN model
    model <- train(diagnosis ~ ., data = brca.normal.training, method = 'knn', trControl = trainControl(method = "none"), tuneGrid = expand.grid(k = k_values[j]))
    
    # Make predictions on the test set
    predictions <- predict(model, newdata = brca.normal.test)
    
    # Calculate accuracy for the current iteration and k value
    accuracy <- mean(predictions == brca.normal.test$diagnosis)
    
    # Add accuracy to the vector for the corresponding k value
    ...
    if (accuracy > best_accuracy) {
      ...
      best_confusion_matrix <- confusionMatrix(predictions, brca.normal.test$diagnosis)
    }
  }
}

# Calculate the average accuracies for each k value
average_accuracies <- accuracies / num_iterations

# Print the average accuracies with the corresponding k values
for (j in 1:length(k_values)) {
  cat("Average Accuracy for k =", k_values[j], ":", average_accuracies[j], "\n")
}
```
As we can see,  the best k is 7 (but the difference is not significant between the diffrent k values)
```{r}
# Print the confusion matrix for the best k value
cat("\nBest k value:", best_k, "\n")
cat("Confusion Matrix:\n")
print(best_confusion_matrix)

```
Next we will perform knn on the dimentionaly reduced data:
...
for (i in 1:num_iterations) {
  ...
  # Subset training set with index
  brca.reduce.training <- shuffled_data[index, ]
  
  # Subset test set with index
  brca.reduce.test <- shuffled_data[-index, ]
  
  # Train a model for each k value
  for (j in 1:length(k_values)) {
    # Train a KNN model
    model <- train(diagnosis ~ ., data = brca.reduce.training, method = 'knn', trControl = trainControl(method = "none"), tuneGrid = expand.grid(k = k_values[j]))
    
    # Make predictions on the test set
    predictions <- predict(model, newdata = brca.reduce.test)
    
    # Calculate accuracy for the current iteration and k value
    accuracy <- mean(predictions == brca.reduce.test$diagnosis)
    ...
    if (accuracy > best_accuracy) {
      best_k <- k_values[j]
      best_accuracy <- accuracy
      best_confusion_matrix <- confusionMatrix(predictions, brca.reduce.test$diagnosis)
    }
  }
}
...
As we can see,  the best k is  3 (but the difference is not significant between the different k values)
```{r}
# Print the confusion matrix for the best k value
cat("\nBest k value:", best_k, "\n")
cat("Confusion Matrix:\n")
...
# Calculate accuracy
accuracy <- sum(predicted_labels == test_target_numeric) / length(test_target_numeric)
...
xgb_model_reduce <- xgboost(data = as.matrix(train_features_reduce), label = train_target_numeric, nrounds = 200, objective = "binary:logistic")
