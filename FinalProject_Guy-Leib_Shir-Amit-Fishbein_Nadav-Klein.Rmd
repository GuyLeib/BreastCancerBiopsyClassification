...
for (n in num_trees) {
  for (d in max_depth) {
    ...
    confusion <- confusionMatrix(predictions, test_target_normal)
...
for (n in num_trees_reduce) {
  for (d in max_depth_reduce) {
    ...
    confusion <- confusionMatrix(predictions_reduce, test_target_reduce)
    sensitivity_reduce_rf <- confusion$byClass["Sensitivity"]
    
    result_reduce <- list(ntrees = n, maxdepth = d, model = model_reduce, predictions = predictions_reduce, accuracy = accuracy_reduce ,sensitivity = sensitivity_reduce_rf)
