...
for (i in 1:num_iterations) {
  # Shuffle the data
  shuffled_data <- brca[sample(nrow(brca)), ]
  ...
  brca.training <- shuffled_data[index, ]
  
  # Subset test set with index
  brca.test <- shuffled_data[-index, ]
  
  # Train a model for each k value
  for (j in 1:length(k_values)) {
    # Train a KNN model
    model <- train(diagnosis ~ ., data = brca.training, method = 'knn', trControl = trainControl(method = "none"), tuneGrid = expand.grid(k = k_values[j]))
    
    # Make predictions on the test set
    predictions <- predict(model, newdata = brca.test)
    
    # Calculate accuracy for the current iteration and k value
    accuracy <- mean(predictions == brca.test$diagnosis)
    ...
    if (accuracy > best_accuracy) {
      ...
      best_confusion_matrix <- confusionMatrix(predictions, brca.test$diagnosis)
    }
  }
}
...
```
Next we will perform knn on the dimentianly reduced data:
...
As we can see,  the best k is 5 (but the difference is not significant between the different k values)
```{r}
# Print the confusion matrix for the best k value
cat("\nBest k value:", best_k, "\n")
cat("Confusion Matrix:\n")
