...
# Confusion matrix 
confusionMatrix(predictions,brca.normal.test[,1], positive="Malignant")
```
...
# Confusion matrix 
confusionMatrix(predictions,brca.reduced.test[,1], positive="Malignant")
...
# Confusion matrix 
confusionMatrix(data=y_pred1, reference=brca.normal.test.labels, positive="Malignant")
```
```{r}
...
# Confusion matrix 
confusionMatrix(data=y_pred1, reference=brca.x.reduce.test.labels, positive="Malignant")
```
...
```{r, echo=FALSE}
# Extract the details of the first tree into a data frame
tree_details <- xgb.model.dt.tree(model = xgb_model_reduce, trees = 2)

# Save the data frame to a CSV file
write.csv(tree_details, file = "tree_details2.csv")
...
```
```{r}
roc_obj <- roc(test_target_numeric, predictions)
plot(roc_obj)
...
grid <- expand.grid(x1 = seq(min(train_features_reduce[, feature1]), max(train_features_reduce[, feature1]), length.out = 100),
                    x2 = seq(min(train_features_reduce[, feature2]), max(train_features_reduce[, feature2]), length.out = 100))

# Create a copy of the grid with proper names for prediction
grid_for_prediction <- grid
colnames(grid_for_prediction) <- colnames(train_features_reduce)[c(feature1, feature2)]

# Add the remaining features with mean values for prediction
for (i in setdiff(1:ncol(train_features_reduce), c(feature1, feature2))) {
  grid_for_prediction[, colnames(train_features_reduce)[i]] <- mean(train_features_reduce[, i])
}

# Predict the class labels for the grid
grid$prediction <- predict(xgb_model_reduce, as.matrix(grid_for_prediction))

# Plot the decision surface
ggplot(grid, aes(x = x1, y = x2, fill = prediction)) +
  geom_tile() +
  scale_fill_gradient(low = "blue", high = "red") +
  labs(x = colnames(train_features_reduce)[feature1], y = colnames(train_features_reduce)[feature2], fill = "Prediction")
...
```{r}
# Compare the accuracies
performance_ <- data.frame(Dataset = c("brca.normal", "brca.reduce"),
                              KNN = c(1,1),
                              SVM = c(1,1),
                              Random_Forest = c(0.9473684,0.9736842),
                              XGBOOST = c(accuracy_normal, accuracy_reduce))
performance_
```
