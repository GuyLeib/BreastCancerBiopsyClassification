...
# Subset training set with index
brca.normal.training <- shuffled_data[index,]
brca.normal.training.labels <- as.factor(brca.normal.training[,1])
brca.normal.test <- shuffled_data[-index,]
brca.normal.test.labels <- as.factor(brca.normal.test[,1])
...
# Svm's accuracy
print((paste('accuracy:',sum(y_pred1 == brca.normal.test.labels)/length(brca.normal.test.labels))))
...
brca.reduce.training.labels <- as.factor(brca.reduce.training[,1])
brca.reduce.test <- shuffled_data[-index,]
brca.reduce.test.labels <- as.factor(brca.reduce.test[,1])
regressor_svm <- svm(formula = brca.reduce.training.labels ~ ., 
                    data=brca.reduce.training,
                    type = 'C-classification',
                    kernel = 'linear')
y_pred1 = predict(regressor_svm, newdata = brca.reduce.test)
# Svm's accuracy
print((paste('accuracy:',sum(y_pred1 == brca.reduce.test.labels)/length(brca.reduce.test.labels))))
CrossTable(brca.reduce.test.labels, y_pred1, prop.chisq = FALSE, prop.c = FALSE, 
           prop.r = FALSE, dnn = c('actual type', 'predicted type'))
