...
for (n in num_trees_reduce) {
  for (d in max_depth_reduce) {
    ...
    result_reduce <- list(ntrees = n, maxdepth = d, model = model_reduce, predictions = predictions_reduce, accuracy = accuracy_reduce_rf ,sensitivity = sensitivity_reduce_rf, confusion=confusion)
...
## Confusion matrix 
```{r}
print(best_model_reduce$confusion)
```
...
# Compare the accuracies
performance <- data.frame(Dataset = c("brca.normal", "brca.reduce"),
                          Accuracy = c(sensitivity_normal_rf, sensitivity_reduce_rf))
performance

```
We can see that the accuracy of the model that were trained on the data set after feature selection and PCA were higher. The sensitivity stays the same. 
...
# Train the XGBoost model on brca.reduce with the best hyperparameters
...
```{r}
# Convert target to factor and relabel levels
train_target_factor <- as.factor(train_target_numeric)
levels(train_target_factor) <- make.names(levels(factor(train_target_numeric)))
...
# Train the model using the training control and parameter grid
best_model <- train(x = as.matrix(train_features_normal),
...
# You can now use the best_model object to make predictions
predictions <- predict(best_model, newdata = as.matrix(test_features_normal), type="prob")
predicted_labels <- ifelse(predictions[, "X1"] >= 0.5, 1, 0)
...
true_positives <- sum(predicted_labels == 1 & test_target_numeric == 1)
actual_positives <- sum(test_target_numeric == 1)
...
# Define the parameter grid to search
param_grid <- expand.grid(nrounds = c(100, 200),
                          max_depth = c(3, 7),
                          eta = c(0.1,0.3),
                          gamma = c(0,0.2),
                          colsample_bytree = c(0.5, 1),
...
# Compare the accuracies
performance_xgb <- data.frame(Dataset = c("brca.normal", "brca.reduce"),
                              Accuracy = c(sensitivity_xg_normal, sensitivity_xg_reduce))
performance_xgb
