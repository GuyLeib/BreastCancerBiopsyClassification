...
```
```{r}
geom.umap <- umap(brca.subset,
  n_neighbors = 15,
  min_dist = 1, spread = 5
)

geom.umap <- data.frame(
  UMAP1 = geom.umap[, 1],
  UMAP2 = geom.umap[, 2],
  label = brca$diagnosis
)

ggplot(geom.umap, aes(x = UMAP1, y = UMAP2, col = label)) +
  geom_point()
```
```{r}

geom.tsne <- Rtsne(brca.subset,
  pca = FALSE, perplexity = 10,
  theta = 0.0
)

geom.tsne <- data.frame(
  TSNE1 = geom.tsne$Y[, 1],
  TSNE2 = geom.tsne$Y[, 2],
  label = brca$diagnosis)

ggplot(geom.tsne, aes(
  x = TSNE1, y = TSNE2,
  col = label
)) +
  geom_point()
